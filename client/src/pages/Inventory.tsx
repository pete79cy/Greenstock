...
export default function Inventory() {
  ...
  // State management with proper initialization and typing
  ...
  // Fetch plant data with search parameter and improved error handling
  const { data: plants = [], isLoading, isError } = useQuery<Plant[]>({
    ...
    queryFn: async () => {
      try {
        ...
        const url = `/api/plants${debouncedSearchTerm ? `?search=${encodeURIComponent(debouncedSearchTerm)}` : ''}`;
  ...
  // Function to filter plants based on filters (year and quantity) with proper error handling
  const filteredPlants = plants && plants.length > 0 ? plants.filter((plant) => {
    if (!plant) return false;
    
    try {
      // Apply year filter
      const matchesYear = yearFilter === "all" || 
                          (plant.plantingYear && plant.plantingYear.toString() === yearFilter);
